...
`Scripts/.behaviour` を作成する。ゲームがアドオンをロードするときの挙動を制御するファイルです。

```json
{
    "LoadRoles": false,
    "UseHiddenMembers": false
}
```

- `LoadRoles`: `true` にすると、アドオンのアセンブリ内で `DefinedAssignable`（役職・モディファイアなど）を実装したクラスの静的コンストラクタをゲームが自動で呼び出し、役職をゲームに登録します。役職を追加するアドオンでは **必ず `true` にしてください**。役職を追加しないアドオンでは `false` のままで構いません。
- `UseHiddenMembers`: 非推奨オプションです。詳細は `/nos-hidden-members` を参照してください。
